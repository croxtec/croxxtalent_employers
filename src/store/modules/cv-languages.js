/* src/store/modules/cv-languages.js */
  ...
  actions: {
    ...
    async list({ commit }, { cv_id, payload }) {
      try {
        ...
        let response = await $http.get(`/cvs/${cv_id}/languages`, {
          params: payload
        });
    ...
    async create({ commit }, { cv_id, payload }) {
      commit("SET_LOADING");
      try {
        let response = await $http.post(`/cvs/${cv_id}/languages`, payload);
        let responsePayload = response.data;
    ...
    async view({ commit }, { cv_id, work_experience_id }) {
      ...
      try {
        let response = await $http.get(
          `/cvs/${cv_id}/languages/${work_experience_id}`
        );
        let responsePayload = response.data;
        commit("SET_DATA", responsePayload);
        commit("SET_SUCCESS", responsePayload.message);
