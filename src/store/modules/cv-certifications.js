/* src/store/modules/cv-certifications.js */
  ...
  actions: {
    ...
    async list({ commit }, { cv_id, payload }) {
      try {
        ...
        let response = await $http.get(`/cvs/${cv_id}/certifications`, {
          params: payload
        });
    ...
    async view({ commit }, { cv_id, work_experience_id }) {
      ...
      try {
        let response = await $http.get(
          `/cvs/${cv_id}/certifications/${work_experience_id}`
        );
        let responsePayload = response.data;
        commit("SET_DATA", responsePayload);
        commit("SET_SUCCESS", responsePayload.message);
